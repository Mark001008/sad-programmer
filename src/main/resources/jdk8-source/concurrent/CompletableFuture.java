...
/**
 * CompletableFuture 源码阅读版。
 *
 * <p>核心模型：result 保存完成结果或异常，stack 保存依赖当前 Future 的后续动作。
 * 当前 Future 完成后，通过 postComplete 触发依赖链继续执行。</p>
 */
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
     ...
     * 完成结果。
     *
     * <p>null 表示未完成；普通对象表示正常结果；AltResult 表示 null、异常或取消。</p>
     */
     ...
     * 依赖动作栈。
     *
     * <p>thenApply、thenCombine、whenComplete 等后续动作会被包装成 Completion 挂到 stack 上。</p>
     */
    ...
    static final class AltResult {
         ...
         * 异常对象。
         *
         * <p>ex 为 null 时代表正常完成但结果为 null。</p>
         */
        final Throwable ex;
    ...
    public static <U> CompletableFuture<U> supplyAsync(Supplier<U> supplier) {
        // 使用默认异步线程池，通常是 ForkJoinPool.commonPool。
        // 企业项目更推荐使用带业务命名、隔离和监控的自定义线程池。
    ...
    public T get() throws InterruptedException, ExecutionException {
        // Future 接口语义：阻塞等待，异常会包装成 ExecutionException，响应中断。
        Object r;
    ...
    public T join() {
        // join 不要求显式处理受检异常，异常会包装成 CompletionException。
        Object r;
    ...
    public <U> CompletableFuture<U> thenCompose(
        ...
        // 扁平化异步编排：上一步结果决定下一步异步任务，避免 Future<Future<T>>。
    ...
    public static CompletableFuture<Void> allOf(CompletableFuture<?>... cfs) {
        // 等待全部 Future 完成。返回值不包含各 Future 的业务结果，需要调用方自己再 join/get。
        return andTree(cfs, 0, cfs.length - 1);
    }


    public static CompletableFuture<Object> anyOf(CompletableFuture<?>... cfs) {
        // 任意一个 Future 完成就完成，常用于多副本查询取最快结果。
        return orTree(cfs, 0, cfs.length - 1);
    }


    public boolean cancel(boolean mayInterruptIfRunning) {
        // CompletableFuture 的 cancel 只是把结果设置为取消异常。
        // mayInterruptIfRunning 在这里没有像 FutureTask 那样直接中断 runner 的效果。
